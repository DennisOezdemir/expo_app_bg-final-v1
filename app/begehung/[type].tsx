...
export default function BegehungScreen() {
  ...
  const togglePosStatus = useCallback((posId: string, newStatus: PosCheckStatus) => {
    ...
    setPosStates((prev) => {
      ...
      return {
        ...
        [posId]: {
          ...current,
          status: current.status === newStatus ? "none" : newStatus,
        },
      };
    });
  }, [finalized]);
  ...
  const addPhoto = useCallback((posId: string) => {
    ...
    setPosStates((prev) => {
      ...
      return {
        ...prev,
        [posId]: { ...current, photoCount: current.photoCount + 1 },
      };
    });
    if (Platform.OS === "web") {
      Alert.alert("Foto", "Kamera wird auf dem Ger\u00E4t ge\u00F6ffnet.");
    }
  }, []);
  ...
  const addFromCatalog = useCallback((entry: CatalogEntry) => {
    ...
    setMehrleistungen((prev) => ({
      ...prev,
      [catalogModalRoom]: [...(prev[catalogModalRoom] || []), item],
    }));
    closeCatalogModal();
  ...
  const addFreeEntry = useCallback(() => {
    ...
    const item: MehrleistungItem = {
      ...
      unit: freeUnit || "Stk",
      price: parseFloat(freePrice.replace(",", ".")) || 0,
      trade: freeTrade.trim() || "Allg.",
      fromCatalog: false,
    };
    setMehrleistungen((prev) => ({
      ...prev,
  ...
  const filteredCatalogEntries = useMemo(() => {
    ...
    return catalog.entries.filter(
      (e) => e.title.toLowerCase().includes(q) || e.desc.toLowerCase().includes(q) || e.trade.toLowerCase().includes(q)
    );
    ...
    <View style={s.container}>
      ...
      <View style={s.summaryBar}>
        <View style={s.summaryItem}>
          ...
          <Text style={[s.summaryCount, { color: Colors.raw.emerald500 }]}>{summary.confirmed}</Text>
        </View>
        <View style={s.summaryItem}>
          <Ionicons name="close-circle" size={16} color={Colors.raw.rose500} />
          <Text style={[s.summaryCount, { color: Colors.raw.rose500 }]}>{summary.rejected}</Text>
        </View>
        <View style={s.summaryItem}>
          <Ionicons name="radio-button-off" size={16} color={Colors.raw.zinc600} />
          <Text style={[s.summaryCount, { color: Colors.raw.zinc500 }]}>{summary.unchecked}</Text>
        </View>
              ...
              <Pressable style={s.roomHeader} onPress={() => toggleRoom(room.id)}>
                ...
                <View style={s.roomInfo}>
                  ...
                  <Text style={s.roomMeta}>
                    {roomDone}/{room.positions.length} gepr{"\u00FC"}ft
                    {roomMehr.length > 0 ? ` \u00B7 ${roomMehr.length} Mehrleistung${roomMehr.length > 1 ? "en" : ""}` : ""}
                  </Text>
                </View>
                <View style={s.roomProgress}>
                  {roomConfirmed > 0 && (
                    <View style={[s.roomDot, { backgroundColor: Colors.raw.emerald500 }]}>
                      <Text style={s.roomDotText}>{roomConfirmed}</Text>
                    </View>
                  )}
                  {roomRejected > 0 && (
                    <View style={[s.roomDot, { backgroundColor: Colors.raw.rose500 }]}>
                      <Text style={s.roomDotText}>{roomRejected}</Text>
                    </View>
                  )}
                </View>
                <Ionicons
                    ...
                    return (
                      <View key={pos.id} style={s.posRow} testID={`pos-${pos.id}`}>
                        <View style={s.posActions}>
                          <Pressable
                            onPress={() => togglePosStatus(pos.id, "confirmed")}
                            style={({ pressed }) => [
                              s.posActionBtn,
                              ps.status === "confirmed" && s.posActionConfirmed,
                          ...
                          >
                            <Ionicons
                              name="checkmark"
                              size={18}
                              color={ps.status === "confirmed" ? Colors.raw.white : Colors.raw.zinc600}
                            />
                          </Pressable>
                          <Pressable
                            onPress={() => togglePosStatus(pos.id, "rejected")}
                            style={({ pressed }) => [
                              s.posActionBtn,
                              ps.status === "rejected" && s.posActionRejected,
                              { opacity: pressed ? 0.7 : 1 },
                            ]}
                            testID={`reject-${pos.id}`}
                          >
                            <Ionicons
                              name="close"
                              size={18}
                              color={ps.status === "rejected" ? Colors.raw.white : Colors.raw.zinc600}
                            />
                          </Pressable>
                        </View>
                        <View style={s.posBody}>
                          <View style={s.posTitleRow}>
                            ...
                            <Text
                              style={[
                                s.posTitle,
                                ps.status === "rejected" && s.posTitleRejected,
                              ]}
                              numberOfLines={1}
                            >
                              {pos.title}
                        ...
                        <Pressable
                          onPress={() => addPhoto(pos.id)}
                          style={({ pressed }) => [s.posPhotoBtn, { opacity: pressed ? 0.7 : 1 }]}
                          testID={`photo-${pos.id}`}
                        >
                          <Ionicons name="camera" size={18} color={Colors.raw.zinc500} />
                          {ps.photoCount > 0 && (
                            <View style={s.photoBadge}>
                              <Text style={s.photoBadgeText}>{ps.photoCount}</Text>
                            </View>
                          )}
                        </Pressable>
                      </View>
                    ...
                    <View style={s.mehrSection}>
                      ...
                      {roomMehr.map((ml) => (
                        <View key={ml.id} style={s.mehrRow}>
                          <View style={s.mehrIconWrap}>
                            <Ionicons name="add-circle" size={16} color={Colors.raw.amber500} />
                          </View>
                          <View style={s.mehrBody}>
                            <Text style={s.mehrTitle}>{ml.title}</Text>
                            <Text style={s.mehrMeta}>
                              {ml.qty} {ml.unit} {"\u00B7"} {formatEuro(ml.price)}/{ml.unit} {"\u00B7"} {ml.trade}
                              {ml.fromCatalog ? "" : " \u00B7 Freitext"}
                            </Text>
                          </View>
                          <Pressable
                            onPress={() => removeMehrleistung(room.id, ml.id)}
                            style={({ pressed }) => ({ opacity: pressed ? 0.5 : 1, padding: 4 })}
                          >
                            <Ionicons name="trash-outline" size={16} color={Colors.raw.zinc600} />
                          </Pressable>
                        </View>
                      ))}
                    </View>
      ...
      >
        ...
        >
          <Pressable style={s.modalOverlay} onPress={closeCatalogModal} />
            ...
            {catalogStep === "pick" && (
              <>
                ...
                {Object.entries(CATALOGS).map(([key, cat]) => (
                  <Pressable
                    key={key}
                    style={({ pressed }) => [s.catalogOption, { opacity: pressed ? 0.7 : 1 }]}
                    onPress={() => {
                  ...
                  >
                    <View style={s.catalogOptionIcon}>
                      <Ionicons name="book" size={20} color={Colors.raw.amber500} />
                    </View>
                    <View style={s.catalogOptionText}>
                      <Text style={s.catalogOptionLabel}>{cat.label}</Text>
                      <Text style={s.catalogOptionDesc}>{cat.entries.length} Positionen</Text>
                    </View>
                ...
                <Pressable
                  style={({ pressed }) => [s.catalogOption, { opacity: pressed ? 0.7 : 1 }]}
                  onPress={() => setCatalogStep("free")}
                >
                  <View style={s.catalogOptionIcon}>
                    <Ionicons name="create" size={20} color={Colors.raw.amber500} />
                  </View>
                  <View style={s.catalogOptionText}>
                    <Text style={s.catalogOptionLabel}>Freitext</Text>
                    <Text style={s.catalogOptionDesc}>Position manuell erfassen</Text>
                  </View>
                  <Ionicons name="chevron-forward" size={18} color={Colors.raw.zinc600} />
                </Pressable>
            ...
            {catalogStep === "browse" && (
              <>
                ...
                <View style={s.browseHeader}>
                  <Pressable onPress={() => setCatalogStep("pick")} style={{ padding: 4 }}>
                ...
                <View style={s.searchWrap}>
                  ...
                  {catalogSearch.length > 0 && (
                    <Pressable onPress={() => setCatalogSearch("")}>
                      <Ionicons name="close-circle" size={18} color={Colors.raw.zinc600} />
                    </Pressable>
                  )}
                </View>
                <ScrollView style={s.catalogList} showsVerticalScrollIndicator={false}>
                  {filteredCatalogEntries.length === 0 && (
                    <Text style={s.catalogEmpty}>Keine Treffer</Text>
                  )}
                  {filteredCatalogEntries.map((entry) => (
                    <Pressable
                      key={entry.id}
                      style={({ pressed }) => [s.catalogEntry, { opacity: pressed ? 0.7 : 1 }]}
                      onPress={() => addFromCatalog(entry)}
                    >
                      <View style={s.catalogEntryBody}>
                        <View style={s.catalogEntryTitleRow}>
                          <Text style={s.catalogEntryNr}>{entry.nr}</Text>
                          <Text style={s.catalogEntryTitle} numberOfLines={1}>{entry.title}</Text>
                        </View>
                        <Text style={s.catalogEntryDesc} numberOfLines={1}>{entry.desc}</Text>
                        <View style={s.catalogEntryMeta}>
                          <Text style={s.catalogEntryPrice}>{formatEuro(entry.price)}/{entry.unit}</Text>
                          <View style={s.posTradeBadge}>
                            <Text style={s.posTradeText}>{entry.trade}</Text>
                          </View>
                        </View>
                      </View>
            ...
            {catalogStep === "free" && (
              <>
                ...
                <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                  ...
                  <TextInput
                    style={s.fieldInput}
                    placeholder="z.B. Buntbartschloss tauschen"
                    placeholderTextColor={Colors.raw.zinc600}
                    value={freeTitle}
                  ...
                  <View style={s.fieldRow}>
                    <View style={s.fieldHalf}>
                      ...
                      <TextInput
                        ...
                        placeholderTextColor={Colors.raw.zinc600}
                        value={freeQty}
                        onChangeText={setFreeQty}
                        keyboardType="numeric"
                      />
                    </View>
                    <View style={s.fieldHalf}>
                      <Text style={s.fieldLabel}>Einheit</Text>
                      <TextInput
                        style={s.fieldInput}
                        placeholder="Stk"
                        placeholderTextColor={Colors.raw.zinc600}
                        value={freeUnit}
                        onChangeText={setFreeUnit}
                      />
                    </View>
                  </View>
                  <View style={s.fieldRow}>
                    <View style={s.fieldHalf}>
                      <Text style={s.fieldLabel}>Preis/Einheit</Text>
                      <TextInput
                        style={s.fieldInput}
                        placeholder="0,00"
                        placeholderTextColor={Colors.raw.zinc600}
                        value={freePrice}
                        onChangeText={setFreePrice}
                        keyboardType="numeric"
                      />
                    </View>
                    <View style={s.fieldHalf}>
                      <Text style={s.fieldLabel}>Gewerk</Text>
                      <TextInput
                        style={s.fieldInput}
                        placeholder="z.B. Tischler"
                        placeholderTextColor={Colors.raw.zinc600}
                        value={freeTrade}
                        onChangeText={setFreeTrade}
                      />
                    </View>
                  </View>
                  <Pressable
                    style={({ pressed }) => [
                      s.freeSubmitBtn,
                      !freeTitle.trim() && s.freeSubmitDisabled,
                      { opacity: pressed ? 0.8 : 1 },
                    ]}
                    onPress={addFreeEntry}
...
const s = StyleSheet.create({
  ...
  mehrSection: {
    ...
    backgroundColor: Colors.raw.amber500 + "08",
  },
  mehrLabel: {
    fontFamily: "Inter_600SemiBold",
    fontSize: 12,
  ...
  mehrRow: {
    ...
    gap: 8,
    borderBottomWidth: 1,
    borderBottomColor: Colors.raw.zinc800 + "40",
  },
  mehrIconWrap: {
    width: 24,
    alignItems: "center",
  ...
  addMehrBtn: {
    ...
    paddingVertical: 14,
    borderTopWidth: 1,
    borderTopColor: Colors.raw.zinc800 + "60",
  },
  addMehrText: {
    fontFamily: "Inter_600SemiBold",
    fontSize: 13,
