# ARCHITEKTUR.md Ã¢â‚¬â€ Technische Spezifikation

> **Lade diese Datei wenn:** DB-Schemas, Event-Details, Flow-Templates

---

## EVENT-SYSTEM (Kern)

### events Tabelle
```sql
CREATE TABLE events (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  project_id UUID REFERENCES projects(id),
  event_type TEXT NOT NULL,
  payload JSONB NOT NULL DEFAULT '{}'::jsonb,
  
  -- Processing State
  created_at TIMESTAMPTZ DEFAULT now(),
  processed_at TIMESTAMPTZ,  -- NULL = unverarbeitet
  
  -- Idempotenz & Tracking
  idempotency_key TEXT UNIQUE,
  source_system TEXT,
  source_flow TEXT,
  error_log TEXT
);

-- Index fÃƒÂ¼r Sweeper
CREATE INDEX idx_events_unprocessed 
ON events (created_at) 
WHERE processed_at IS NULL;
```

### event_routing Tabelle
```sql
CREATE TABLE event_routing (
  event_type TEXT PRIMARY KEY,
  target_workflow TEXT NOT NULL,
  description TEXT,
  is_active BOOLEAN DEFAULT true
);

-- Beispiel-Routing
INSERT INTO event_routing VALUES
('EMAIL_RECEIVED', 'M1_02_Email_Parser', 'Neue Mail parsen', true),
('PROJECT_CREATED', 'M1_04_Drive_Setup', 'Drive-Ordner anlegen', true),
('DRIVE_SETUP_COMPLETE', 'M1_05_Offer_Draft', 'Angebot vorbereiten', true);
```

---

## FLOW-TEMPLATE (So sieht jeder Flow aus)

```
Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
Ã¢â€â€š INPUT: event_id, project_id (vom Router)Ã¢â€â€š
Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¤
Ã¢â€â€š 1. Daten aus DB laden                   Ã¢â€â€š
Ã¢â€â€š 2. Dateien aus Storage (falls nÃƒÂ¶tig)    Ã¢â€â€š
Ã¢â€â€š 3. EINE Aufgabe erledigen               Ã¢â€â€š
Ã¢â€â€š 4. DB updaten                           Ã¢â€â€š
Ã¢â€â€š 5. Input-Event als processed markieren  Ã¢â€â€š
Ã¢â€â€š 6. Neuen Event erzeugen                 Ã¢â€â€š
Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¤
Ã¢â€â€š OUTPUT: Nichts! Event triggert nÃƒÂ¤chsten Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
```

### n8n Pseudocode
```javascript
// 1. Event laden
const event = await supabase
  .from('events')
  .select('*')
  .eq('id', input.event_id)
  .single();

// 2. Projekt laden
const project = await supabase
  .from('projects')
  .select('*')
  .eq('id', event.project_id)
  .single();

// 3. Arbeit machen
const result = doTheWork(project, event.payload);

// 4. DB updaten
await supabase
  .from('projects')
  .update({ ... })
  .eq('id', project.id);

// 5. Input-Event abschlieÃƒÅ¸en
await supabase
  .from('events')
  .update({ processed_at: new Date().toISOString() })
  .eq('id', event.id);

// 6. Folge-Event erzeugen
await supabase
  .from('events')
  .insert({
    project_id: project.id,
    event_type: 'NEXT_EVENT_TYPE',
    payload: { ... },
    source_flow: 'M1_04_Drive_Setup'
  });
```

---

## MX_00_EVENT_ROUTER (Dispatcher)

```
Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â     Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
Ã¢â€â€š   Webhook    Ã¢â€â€š     Ã¢â€â€š   Sweeper    Ã¢â€â€š
Ã¢â€â€š  (sofort)    Ã¢â€â€š     Ã¢â€â€š (alle 5 Min) Ã¢â€â€š
Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ     Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
       Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
                Ã¢â€“Â¼
       Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
       Ã¢â€â€š Routing laden   Ã¢â€â€š
       Ã¢â€â€š aus event_routingÃ¢â€â€š
       Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
                Ã¢â€“Â¼
       Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
       Ã¢â€â€š Switch on       Ã¢â€â€š
       Ã¢â€â€š event_type      Ã¢â€â€š
       Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ
                Ã¢â€â€š
    Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â¼Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â
    Ã¢â€“Â¼           Ã¢â€“Â¼           Ã¢â€“Â¼
  M1_04      M1_05       M2_xx
```

### Sweeper Query
```sql
SELECT * FROM events
WHERE processed_at IS NULL
  AND created_at < now() - INTERVAL '2 minutes'
ORDER BY created_at ASC
LIMIT 10;
```

---

## IDEMPOTENZ-PATTERN

### Bei Email-Intake
```javascript
const idempotencyKey = crypto
  .createHash('sha256')
  .update(`${messageId}|${subject}|${attachmentHashes.join(',')}`)
  .digest('hex');

// Insert mit ON CONFLICT
await supabase.from('events').insert({
  event_type: 'EMAIL_RECEIVED',
  idempotency_key: idempotencyKey,
  payload: { ... }
}).onConflict('idempotency_key').ignore();
```

---

## STORAGE-PATTERN

### Upload
```javascript
const { data, error } = await supabase.storage
  .from('project-files')
  .upload(`${projectId}/${filename}`, fileBuffer, {
    contentType: 'application/pdf'
  });

// Nur Path in Event
await supabase.from('events').insert({
  event_type: 'PDF_UPLOADED',
  payload: {
    storage_path: data.path,
    filename: filename,
    size_bytes: fileBuffer.length
  }
});
```

### Download (im nÃƒÂ¤chsten Flow)
```javascript
const { data } = await supabase.storage
  .from('project-files')
  .download(event.payload.storage_path);
```

---

## TECH-STACK REFERENZ

| Layer | Tech | Zweck |
|-------|------|-------|
| DB | Supabase PostgreSQL | Single Source of Truth |
| Auth | Supabase Auth | User Management |
| Storage | Supabase Storage | PDFs, Bilder |
| Webhooks | Supabase Database Webhooks | Event-Trigger |
| Automation | n8n (self-hosted) | Flow-Execution |
| Frontend | Expo (React Native) + TypeScript | Mobile App |
| Server | Express.js | API-Endpunkte (Altpfad, wird reduziert) |
| ORM | Drizzle | DB-Zugriff im Server |
| Icons | Lucide | Icons |
| AI | Claude Vision (via n8n) | PDF-Parsing, Klassifizierung |
| PDF | Gotenberg | PDF-Generierung |
| Dev | Claude Code + Cursor | IDE |
